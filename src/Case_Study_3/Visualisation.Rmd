...
The plot shows that income status is unevenly distributed across continents. Africa has the highest number of low- and lower-middle-income entities, while Europe and the Americas are dominated by high-income entities. Asia shows a more balanced distribution across income levels, and Oceania has fewer entities overall, mostly in the high- and upper-middle-income groups. This highlights global disparities in economic development.
...
This plot shows the proportion of each income group within every continent. Africa is dominated by low- and lower-middle-income entities, with very few high-income countries. In contrast, Europe and the Americas have a much higher proportion of high-income entities. Asia displays a more balanced distribution across all income categories. Oceania is also primarily composed of high- and upper-middle-income entities. The presence of the “NA” category in some regions indicates missing or unclassified data. This plot allows for easier comparison of the internal income structure within continents, independent of their overall size.
...
mosaicplot(~ continent + income_status, data = data_filtered, color = TRUE,
           ...
           xlab = "Continent", ylab = "Income Status")
````
