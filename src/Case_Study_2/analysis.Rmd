...
```{r import-edu-clean}
# Read the lines from the file, skipping the first 2
edu_lines <- readLines(file.path(data_path, "rawdata_369.txt"))[-c(1, 2)]

# Extract country name and expenditure value using regular expressions
edu_exp <- tibble(
  country = str_trim(str_replace(edu_lines, "^\\d+\\s+", "") %>% 
                     str_extract("^[A-Za-z .,'()\\-]+")),
  education_expenditure = as.numeric(str_extract(edu_lines, "(\\d{1,2}\\.\\d{1,2})(?=\\s+\\d{4})"))
)
```
...
# Extract country and net migration rate
net_mig <- tibble(
  country = str_trim(str_extract(mig_lines, "^[0-9]+\\s+(.*?)\\s{2,}")),
  net_migration = as.numeric(str_extract(mig_lines, "-?\\d+\\.\\d+"))
)
```
...
```{r merge-data}
# Merge the three datasets using full_join on 'country'

merged_data <- edu_exp %>%
  full_join(youth_unemp, by = "country") %>%
  full_join(net_mig, by = "country")
```
