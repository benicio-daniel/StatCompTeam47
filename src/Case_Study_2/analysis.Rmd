...
Here are the criteria for untidy data:
• Column headers are values, not variable names. (Columns are variable names✅)
• Multiple types of observational units are stored in the same table.(Only Countries are observational units✅), unless you count a obbservational unit as a countriy or year
• A single observational unit is stored in multiple tables. (One table ✅)
...
Here we simply check, if a singular values do not cohere with the rest of the values in any column.(Doesnt seem to be✅)
```{r}
